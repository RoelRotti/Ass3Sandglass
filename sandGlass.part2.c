...
/* 'compare' compares two states, returns -1 if a procedes b, 0 if a is identical to b, 1 is b procedes a */

int compare(State one, State two){
			/* if time of state one is smaller return -1; */
			if (one.time < two.time)
			{
				return -1;
			} 
			/* if time of state two is smaller return 1; */
			else if (one.time > two.time)
			{
				return 1;
			} 
			/* if time is equal compare rest of the values: 
			 * compare the values of sg1: */
			else if(one.sg1 < two.sg1)
			{
				return -1;
			} 
			else if (one.sg1 > two.sg1)
			...
			/* compare the values of sg2: */
			else if (one.sg2 < two.sg2)
			{
				return -1;
			} 
			else if (one.sg2 > two.sg2)
   ...
   /* The function insertUnique ensures the program doesn't place two identical states in the queue*/
...
void insertUnique(State s, Queue *qp){
			...
			list->next = addItem(s, NULL);
			qp->lastNode = list->next;
}
...
int timeable(int cap1, int cap2, int goalTime) { 
			...
			/* check for simplest cases */
	 		if(goalTime == 0 || goalTime%cap1 == 0 || goalTime%cap2 == 0 || goalTime%(cap1 + cap2) == 0)
	 		{
	 			return 1;
	 		}
	 		...
	 		while( !isEmptyQueue(q) )
	 		{
	 			...
	 			if (reachedGoal.time == goalTime)
	 			{
	 				...
	 				{
	 					insertUnique(action(reachedGoal,1,cap1,cap2), &q); 
	 				}
	 				insertUnique(action(reachedGoal,2,cap1,cap2), &q);
	 				insertUnique(action(reachedGoal,3,cap1,cap2), &q); 
	 				insertUnique(action(reachedGoal,4,cap1,cap2), &q);				
	 			}
